...
# print(npatoms)
# for i in range(npatoms):
    # xx=np.where(abs(atomdata[npnumber,4]-atomdata[npnumber[i],4])<1e-5)[0]
    # yy=np.where(abs(atomdata[npnumber,5]-atomdata[npnumber[i],5])<1e-5)[0]
    # zz=np.where(abs(atomdata[npnumber,6]-atomdata[npnumber[i],6])<1e-5)[0]
    # xv=np.setdiff1d(np.intersect1d(yy,zz),i);
    # yv=np.setdiff1d(np.intersect1d(xx,zz),i);
    # zv=np.setdiff1d(np.intersect1d(xx,yy),i);
    # if((xv.size!=0)&(yv.size!=0)&(zv.size!=0)):
        # vv=i;xv=xv[0];yv=yv[0];zv=zv[0];
        # print(vv,xv,yv,zv)
        # break;
...
y.box(lx=str(float(xhi)-float(xlo)+0.01),ly=str(float(yhi)-float(ylo)+0.01),lz=str(float(zhi)-float(zlo)+0.01))
